...
int game_loop(struct World* world) {
    ...
    while (!quit){
        ...
        if (pause) {
            ...
            //draw button text
            for (int i = 0; i < 4; i++) {
                v2_f letter_xy = get_sprite_coordinate(menu_letters[i]);
                SDL_Rect srcRect = (SDL_Rect) {letter_xy.x, letter_xy.y, 8,8};
                SDL_Rect dstRect = (SDL_Rect) {rect.x+FONT_SIZE*(i+1)+1,
                    rect.y+(BUTTON_HEIGHT - FONT_SIZE)/2,FONT_SIZE,FONT_SIZE};
